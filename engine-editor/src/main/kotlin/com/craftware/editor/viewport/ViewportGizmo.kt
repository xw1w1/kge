...
object ViewportGizmo {
    ...
    fun render(viewProj: Matrix4f, pos: Vector3f, activeAxis: Axis?, cameraPos: Vector3f) {
        if (!initialized) return
        shader.bind()
        ...
        fun draw(mesh: GLMesh, color: Vector3f, rotation: Matrix4f = Matrix4f(), s: Float = 1f) {
            val model = Matrix4f().translate(pos).mul(rotation).scale(s)
        ...
        val colX = if (activeAxis == Axis.X) Vector3f(1f, 0.65f, 0.65f) else Vector3f(1f, 0f, 0f)
        val colY = if (activeAxis == Axis.Y) Vector3f(0.65f, 1f, 0.65f) else Vector3f(0f, 1f, 0f)
        val colZ = if (activeAxis == Axis.Z) Vector3f(0.65f, 0.65f, 1f) else Vector3f(0f, 0f, 1f)
        val colC = if (activeAxis == Axis.CENTER) Vector3f(1f, 1f, 0.5f) else Vector3f(1f, 1f, 0f)

        val colPlaneXY = if (activeAxis == Axis.PLANE_XY) Vector3f(0.65f, 0.65f, 0.90f) else Vector3f(0f, 0f, 0.6f)
        val colPlaneXZ = if (activeAxis == Axis.PLANE_XZ) Vector3f(0.65f, 0.90f, 0.65f) else Vector3f(0f, 0.6f, 0f)
        val colPlaneYZ = if (activeAxis == Axis.PLANE_YZ) Vector3f(0.90f, 0.65f, 0.65f) else Vector3f(0.6f, 0f, 0f)
