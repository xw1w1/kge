...
object EditorDragManager {
    ...
    private var active: DragPayload<*>? = null
    ...
    fun <T : Any> beginDragCandidate(type: KClass<T>, data: T, label: String) {
        ...
        pending = PendingCandidate(type, data as Any, label, ImVec2(min.x, min.y), ImVec2(max.x, max.y))
    }

    fun <T : Any> getPayload(): DragPayload<T>? {
        @Suppress("UNCHECKED_CAST")
        return active as? DragPayload<T>?
    }
    ...
    fun processPendingAndRender() {
        ...
        if (!dragActive && pending != null) {
            ...
            if (dx * dx + dy * dy > DRAG_THRESHOLD * DRAG_THRESHOLD && ImGui.isMouseDown(0)) {
                val s = pending!!
                @Suppress("UNCHECKED_CAST")
                active = DragPayload(s.type as KClass<Any>, s.data, s.label)
                dragActive = true
                pending = null
