...
class ViewportGizmoRenderer {
    ...
    fun init(shaderProgram: ShaderProgram) {
        ...
        centerSphere = buildSphereMesh(1, 1)
    ...
    private fun buildArrowMesh(): GLMesh {
        ...
        for (i in 0 until segments) {
            idx.add(baseIndex + i); idx.add(baseIndex + i + 1); idx.add(tipIndex)
        }
    ...
    fun render(viewProj: Matrix4f, pos: Vector3f, activeAxis: EditorSelectionGizmoAxis?, cameraPos: Vector3f) {
        ...
        val colX = if (activeAxis == EditorSelectionGizmoAxis.TRANSLATE_X) Vector3f(1f, 0.65f, 0.65f) else Vector3f(1f, 0f, 0f)
        val colY = if (activeAxis == EditorSelectionGizmoAxis.TRANSLATE_Y) Vector3f(0.65f, 1f, 0.65f) else Vector3f(0f, 1f, 0f)
        val colZ = if (activeAxis == EditorSelectionGizmoAxis.TRANSLATE_Z) Vector3f(0.65f, 0.65f, 1f) else Vector3f(0f, 0f, 1f)
        val colC = if (activeAxis == EditorSelectionGizmoAxis.TRANSLATE_CENTER) Vector3f(1f, 1f, 0.5f) else Vector3f(1f, 1f, 0f)

        val colPlaneXY = if (activeAxis == EditorSelectionGizmoAxis.TRANSLATE_PLANE_XY) Vector3f(0.65f, 0.65f, 0.90f) else Vector3f(0f, 0f, 0.6f)
        val colPlaneXZ = if (activeAxis == EditorSelectionGizmoAxis.TRANSLATE_PLANE_XZ) Vector3f(0.65f, 0.90f, 0.65f) else Vector3f(0f, 0.6f, 0f)
        val colPlaneYZ = if (activeAxis == EditorSelectionGizmoAxis.TRANSLATE_PLANE_ZY) Vector3f(0.90f, 0.65f, 0.65f) else Vector3f(0.6f, 0f, 0f)
