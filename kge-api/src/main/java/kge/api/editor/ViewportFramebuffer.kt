...
class ViewportFramebuffer {
    ...
    fun ensureFramebuffer(width: Int, height: Int, internalFormat: Int = GL11.GL_RGB) {
        ...
        GL11.glTexImage2D(GL11.GL_TEXTURE_2D, 0, internalFormat, width, height, 0, GL11.GL_RGB, GL11.GL_UNSIGNED_BYTE, 0L)
        ...
        require(status == GL30.GL_FRAMEBUFFER_COMPLETE) {
    ...
    fun dispose() {
        if (buffer != 0) GL30.glDeleteFramebuffers(buffer)
